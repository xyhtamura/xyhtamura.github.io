...
Here is the content formatted in raw markdown:
## **Portfolio Summary**

**seismocyte** is an interactive, browser-based audio-visual instrument that translates real-time global tectonic activity into generative soundscapes and visual topography. By feeding live earthquake data from the USGS API into a custom physics and audio engine, it maps seismic magnitude and focal depth directly to bathyal sub-harmonics, physical collisions, and fluid friction. The result is a living geological interface—a digital tissue sample reacting autonomously to the grinding of the earth's crust.
...
The extracted data directly manipulates both the 2D canvas physics and the Web Audio API synthesis engine.
  ...
  - **Magnitude → Tectonic Force (boost)**  
    The magnitude dictates the kinetic energy injected into the system. A baseline 4.0 magnitude triggers a subtle, localized twitch. As the magnitude scales toward 7.0+, the force multiplies drastically, violently tearing the "seismocytes" apart and increasing the overall friction/crackle envelope of the audio.
  - **Focal Depth → Resonant Frequency (depthModifier)**  
    The depth of the quake modifies the pitch of the scraping audio voices. Shallow quakes maintain a higher, tearing friction sound. Deep-crust events (e.g., depths approaching 500km+) pull the resonant filters and triangle-wave oscillators downward, resulting in a heavy, bowel-shaking sub-bass groan.
